...
void ArgumentShuffle::pd_generate(MacroAssembler* masm, int in_stk_bias, int out_stk_bias) const {
  for (int i = 0; i < _moves.length(); i++) {
    ...
    switch (arg_bt) {
      ...
      case T_CHAR:
      case T_INT:
        masm->move32_64(from_vmreg, to_vmreg, in_stk_bias, out_stk_bias);
        break;
      ...
      case T_FLOAT:
        if (to_vmreg.first()->is_Register()) { // Windows vararg call
          ...
          masm->float_move(from_vmreg, to_vmreg, in_stk_bias, out_stk_bias);
        }
        break;
      ...
      case T_DOUBLE:
        if (to_vmreg.first()->is_Register()) { // Windows vararg call
          ...
          masm->double_move(from_vmreg, to_vmreg, in_stk_bias, out_stk_bias);
        }
        break;

      case T_LONG:
        masm->long_move(from_vmreg, to_vmreg, in_stk_bias, out_stk_bias);
        break;
