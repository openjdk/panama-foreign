...
public:
  DowncallStubGenerator(CodeBuffer* buffer,
   ...
   : StubCodeGenerator(buffer, PrintMethodHandleStubs),
     ...
     _preserve_value_mask(preserve_value_mask),
     _frame_complete(0),
     _frame_size_slots(0),
     _oop_maps(NULL) {
  }
...
RuntimeStub* DowncallLinker::make_downcall_stub(BasicType* signature,
  ...
  DowncallStubGenerator g(&code, signature, num_args, ret_bt, abi,
                          ...
                          needs_return_buffer, preserve_value_mask);
  g.generate();
...
void DowncallStubGenerator::generate() {
  ...
  if (_preserve_value_mask != 0) {
    locs.set_frame_data(StubLocations::PRESERVE_VALUE, allocated_frame_size);
    allocated_frame_size += 8;
  }
  ...
  if (_preserve_value_mask != 0) {
    ...
    __ movptr(c_rarg0, Address(rsp, locs.data_offset(StubLocations::PRESERVE_VALUE)));
    __ movl(c_rarg1, _preserve_value_mask);
    __ mov(r12, rsp); // remember sp
    __ subptr(rsp, frame::arg_reg_save_area_bytes); // windows
    __ andptr(rsp, -16); // align stack as required by ABI
    __ call(RuntimeAddress(CAST_FROM_FN_PTR(address, DowncallLinker::preserve_value)));
