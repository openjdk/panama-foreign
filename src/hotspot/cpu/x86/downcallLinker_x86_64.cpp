...
public:
  DowncallStubGenerator(CodeBuffer* buffer,
   ...
   : StubCodeGenerator(buffer, PrintMethodHandleStubs),
     ...
     _save_thread_local_mask(save_thread_local_mask),
     _frame_complete(0),
     _frame_size_slots(0),
     _oop_maps(NULL) {
  }
...
RuntimeStub* DowncallLinker::make_downcall_stub(BasicType* signature,
  ...
  DowncallStubGenerator g(&code, signature, num_args, ret_bt, abi,
                          ...
                          needs_return_buffer, save_thread_local_mask);
  g.generate();
...
void DowncallStubGenerator::generate() {
  ...
  if (_save_thread_local_mask != 0) {
    locs.set_frame_data(StubLocations::SAVE_THREAD_LOCAL, allocated_frame_size);
    allocated_frame_size += 8;
  }
  ...
  if (_save_thread_local_mask != 0) {
    ...
    __ movptr(c_rarg0, Address(rsp, locs.data_offset(StubLocations::SAVE_THREAD_LOCAL)));
    __ movl(c_rarg1, _save_thread_local_mask);
    __ mov(r12, rsp); // remember sp
    __ subptr(rsp, frame::arg_reg_save_area_bytes); // windows
    __ andptr(rsp, -16); // align stack as required by ABI
    __ call(RuntimeAddress(CAST_FROM_FN_PTR(address, DowncallLinker::save_thread_local)));
