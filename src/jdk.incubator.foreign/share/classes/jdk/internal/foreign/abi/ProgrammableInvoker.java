...
public class ProgrammableInvoker {
    ...
    Object invoke(Object[] args) {
        ...
        try (MemorySegment argBuffer = MemorySegment.ofNative(layout.size, 64)) {
            ...
            VH_LONG.set(argsPtr.offset(layout.arguments_next_pc), addr);
            VH_LONG.set(argsPtr.offset(layout.stack_args_bytes), stackArgsBytes);
            VH_LONG.set(argsPtr.offset(layout.stack_args), MemoryAddressImpl.addressof(stackArgs));
                ...
                jdk.internal.foreign.abi.BindingInterpreter.unbox(arg, callingSequence.argumentBindings(i),
                        s -> {
                            if (abi.arch.isStackType(s.type())) {
                                return stackArgs.offset(s.index() * abi.arch.typeSize(abi.arch.stackType()));
                            }
                            return argsPtr.offset(layout.argOffset(s));
                        }, tempBuffers);
            }
            ...
            return function.returnLayout().isEmpty()
                    ...
                    s -> argsPtr.offset(layout.retOffset(s))); // buffers are leaked
        } finally {
