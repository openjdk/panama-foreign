...
public class DirectNativeInvoker {
    public static MethodHandle make(MemoryAddress symbol, CallingSequence callingSequence, MethodType type) {
        DirectSignatureShuffler shuffler =
                DirectSignatureShuffler.javaToNativeShuffler(callingSequence, type);
        MethodHandle mh;
        try {
            ...
            mh = MethodHandles.insertArguments(mh, 0, MemoryAddressImpl.addressof(symbol));
        } catch (Throwable ex) {
