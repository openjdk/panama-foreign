...
/**
 * A binary AST node with an associative operator.
 * Result and operand types are all the same.
 *
 * @param <T> The element type of the result and two operands.
 */
public class MatrixAssociativeBinary<T> extends MatrixAssociative<T> {
    public final A2Expr<T> y;

    public MatrixAssociativeBinary(AssociativeOp<T> op, A2Expr<T> x, A2Expr<T> y) {
        super(op, x, x.preferredMajorVote() + y.preferredMajorVote());
        this.y = y;
    ...
    public A2Expr<T> simplify(Matrix<T> target, Simplifier simplifier) {
        return simplifier.simplifyEltWise2(target, this);
    }
     ...
     * @param blocksize
     */
     void setRecursive(Matrix<T> target, Matrix<T> operand1, Matrix<T> operand2, int blocksize) {
        long n_rows = target.length(ROW_DIM);
        long n_cols = target.length(COL_DIM);
        if (n_rows <= blocksize) {
            if (n_cols <= blocksize) {
     ...
     * @param j_hi
     */
     void setSubMatrix(Matrix<T> target, Matrix<T> operand1, Matrix<T> operand2,
            long i_lo, long i_hi,
            long j_lo, long j_hi) {
        long o1_i_off = 0;
        long o2_i_off = 0;
    ...
    static public class EltWise2Action<T> extends RecursiveAction {
        private static final long serialVersionUID = 0x4c30e2e75b06c077L;

        protected final Matrix<T> target;
        protected final Matrix<T> operand1;
        protected final Matrix<T> operand2;
        protected final MatrixAssociativeBinary<T> node;
        protected final int blocksize;

        public EltWise2Action(MatrixAssociativeBinary<T> node, Matrix<T> target,
                Matrix<T> operand1, Matrix<T> operand2, int blocksize) {
            this.target = target;
