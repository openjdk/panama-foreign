...
/**
 * A ternary AST node with an associative operator.
 *
 * @param <T> The element type of the result and the four operands.
 */
public class MatrixAssociative4ary<T> extends MatrixAssociative<T> {
    // Might be a mistake to do these in alpaabetical order.
    public final A2Expr<T> w;
    // x defined in super, recall.
    public final A2Expr<T> y;
    public final A2Expr<T> z;

    public MatrixAssociative4ary(AssociativeOp<T> op, A2Expr<T> w, A2Expr<T> x, A2Expr<T> y, A2Expr<T> z) {
        super(op, x, w.preferredMajorVote() + x.preferredMajorVote() + y.preferredMajorVote() + z.preferredMajorVote());
        this.w = w;
        this.y = y;
        this.z = z;
        if (x.length(ROW_DIM) != y.length(ROW_DIM) || x.length(ROW_DIM) != z.length(ROW_DIM) || x.length(ROW_DIM) != w.length(ROW_DIM) || x.length(COL_DIM) != w.length(COL_DIM) || x.length(COL_DIM) != y.length(COL_DIM) || x.length(COL_DIM) != z.length(COL_DIM)) {
    ...
    public A2Expr<T> simplify(Matrix<T> target, Simplifier simplifier) {
        return simplifier.simplifyEltWise4(target, this);
    }
    ...
    public void evalInto(Matrix<T> target) {
        ...
        if (w.length(ROW_DIM) != n_rows || x.length(ROW_DIM) != n_rows ||
            y.length(ROW_DIM) != n_rows|| z.length(ROW_DIM) != n_rows  ||
            w.length(COL_DIM) != n_cols || x.length(COL_DIM) != n_cols ||
            y.length(COL_DIM) != n_cols || z.length(COL_DIM) != n_cols) {
            throw new IllegalArgumentException("Matrices must have conforming sizes");
        }
        setRecursive(target,
                w.getValue(), x.getValue(), y.getValue(), z.getValue(),
                A2Expr.elementWiseBlockSize());
    }
     ...
     * @param blocksize
     */
     void setRecursive(Matrix<T> target, Matrix<T> operand1, Matrix<T> operand2, Matrix<T> operand3, Matrix<T> operand4, int blocksize) {
        long n_rows = target.length(ROW_DIM);
        long n_cols = target.length(COL_DIM);
        if (n_rows <= blocksize) {
            if (n_cols <= blocksize) {
                // kernel iteration.
                setSubMatrix(target, operand1, operand2, operand3, operand4, 0, n_rows, 0, n_cols);
            } else {
                // subdivide columns
                long m0 = n_cols >> 1;
                ...
                long m5 = operand4.middle(COL_DIM);
                m0 = A2Expr.firstNotEqual(m0, m1, m2, m3, m4, m5);

                EltWise4Action<T> right = new EltWise4Action<>(
                        this, target.getColumns(m0, n_cols),
                        operand1.getColumns(m0, n_cols),
                        operand2.getColumns(m0, n_cols),
                        operand3.getColumns(m0, n_cols),
                        operand4.getColumns(m0, n_cols), blocksize);
                EltWise4Action<T> left = new EltWise4Action<>(
                        this, target.getColumns(0, m0),
                        operand1.getColumns(0, m0),
                        operand2.getColumns(0, m0),
                        operand3.getColumns(0, m0),
                        operand4.getColumns(0, m0), blocksize);
            ...
            long m5 = operand4.middle(ROW_DIM);
            m0 = A2Expr.firstNotEqual(m0, m1, m2, m3, m4, m5);
            ...
            EltWise4Action<T> right = new EltWise4Action<>(
                    ...
                    operand3.getRows(m0, n_rows),
                    operand4.getRows(m0, n_rows), blocksize);
            EltWise4Action<T> left = new EltWise4Action<>(
                    this, target.getRows(0, m0),
                    operand1.getRows(0, m0),
                    operand2.getRows(0, m0),
                    operand3.getRows(0, m0),
                    operand4.getRows(0, m0), blocksize);
            ForkJoinTask.invokeAll(left, right);
        }
    }
     ...
     * @param j_hi
     */
     void setSubMatrix(Matrix<T> target, Matrix<T> operand1, Matrix<T> operand2, Matrix<T> operand3, Matrix<T> operand4,
            long i_lo, long i_hi,
            long j_lo, long j_hi) {
        long o1_i_off = 0;
        long o2_i_off = 0;
        ...
        if (operand4 instanceof SubMatrix) {
            SubMatrix<T> sub_4 = (SubMatrix<T>) operand4;
            o4_i_off += sub_4.beginRow();
            o4_j_off += sub_4.beginColumn();
            operand4 = sub_4.parent();
        }
        op.setOffsetSubMatrix(target, operand1, operand2, operand3, operand4,
                i_lo, i_hi, o1_i_off, o2_i_off, o3_i_off, o4_i_off,
                j_lo, j_hi, o1_j_off, o2_j_off, o3_j_off, o4_j_off);
    }


    static public class EltWise4Action<T> extends RecursiveAction {
        private static final long serialVersionUID = 0x19bb6baf8a5ce32bL;
        ...
        protected final Matrix<T> operand4;
        protected final MatrixAssociative4ary<T> node;
        protected final int blocksize;

        public EltWise4Action(MatrixAssociative4ary<T> node, Matrix<T> target,
                Matrix<T> operand1, Matrix<T> operand2, Matrix<T> operand3, Matrix<T> operand4, int blocksize) {
            this.target = target;
        ...
        protected void compute() {
            node.setRecursive(target, operand1, operand2, operand3, operand4, blocksize);
        }
    }
}
