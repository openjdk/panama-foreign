...
class NativeInvoker {
    ...
    private static MethodHandle lookupNativeFunction(SymbolLookup lookup, String symbolName) throws NoSuchMethodException, IllegalAccessException {
        return MethodHandles.publicLookup().findNative2(lookup.lookup(symbolName), symbolName);
    }
    ...
    private Object processReturnValue(CallingSequence callingSequence, long[] returnValues, Reference<?> returnStruct) {
        ...
        } else if (Util.isVectorType(methodType.returnType())) {
            return createVector(methodType.returnType(), returnValues);
    ...
    private long[] getVectorValue(Class<?> carrierType, Object arg) {
        if (carrierType == Long2.class) {
            Long2 v = (Long2)arg;
            return new long[] { v.extract(0), v.extract(1) };
        } else if (carrierType == Long4.class) {
            Long4 v = (Long4) arg;
            return new long[]{v.extract(0), v.extract(1), v.extract(2), v.extract(3)};
        } else {
            Long8 v = (Long8) arg;
            return new long[]{v.extract(0), v.extract(1), v.extract(2), v.extract(3), v.extract(4),v.extract(5), v.extract(6), v.extract(7)};
        }
    }

    private Object createVector(Class<?> carrierType, long[] values) {
        switch (values.length) {
            case 2: return Long2.make(values[0], values[1]);
            case 4: return Long4.make(values[0], values[1], values[2], values[3]);
            case 8: return Long8.make(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7]);
            default:
                throw new InternalError("Unsupported vector size: " + values.length);
        }
    }
    ...
    private long[] getArgumentValues(Class<?> carrierType, ArgumentBinding binding, long n, Object arg) throws Throwable {
        ...
        } else if (Util.isVectorType(carrierType)) {
            int index = (int)(binding.getOffset() / 8);
            long[] values = new long[(int)n];
            long[] vectorLongs = getVectorValue(carrierType, arg);

            System.arraycopy(vectorLongs, index, values, 0, (int)n);
            return values;
