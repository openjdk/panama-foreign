...
class HeaderImplGenerator extends ClassGenerator {
    ...
    private static MethodHandle getBridgeTarget(Bridge bridge, MethodType templateMethodType) {
        Class<?> bridgeTarget = bridge.to();

        MethodType methodType = templateMethodType;

        if (!bridge.isConstructor() && bridge.injectPointer()) {
            methodType = templateMethodType.insertParameterTypes(0, Pointer.class);
        }

        try {
            return MethodHandles.publicLookup().findVirtual(bridgeTarget, bridge.name(), methodType);
        } catch (NoSuchMethodException | IllegalAccessException e) {
            throw new RuntimeException("Failed to look up bridge target " + bridge + " using method type " + methodType);
        }
    }

    private void generateBridgeMethod(ClassGeneratorContext ctxt, Method method) {
        ...
        //
        //   Pointer<T> new_(<args>) {
        //      return bridge_target(<args>);
        //   }
        //
        // OR normal instance method:
        //
        //   <ret> m(<args>) {
        //     return bridge_target(p, <args>);
        //   }
        //
        // OR type conversion function
        //
        //  Pointer<T> {static,dynamic}_cast(Pointer<S> p) {
        //     return bridge_target(p);
        //  }
        //

        MethodGenerator generator;

        if (bridge.isConstructor()) {
            generator = new GenericMethodImplGenerator(ctxt, method.getName(), Util.methodTypeFor(method), false, mh, false);
        } else if (bridge.injectPointer()) {
            MethodHandle refPtrMH;
            try {
                refPtrMH = MethodHandles.publicLookup().findVirtual(Reference.class, "ptr", MethodType.methodType(Pointer.class));
            } catch (NoSuchMethodException | IllegalAccessException e) {
                throw new RuntimeException(e);
            }
            mh = MethodHandles.filterArguments(mh, 0, refPtrMH);

            generator = new GenericMethodImplGenerator(ctxt, method.getName(), Util.methodTypeFor(method), false, mh, true);
        } else {
            // Drop the injected dummy/static (reference) argument
            mh = MethodHandles.dropArguments(mh, 0, Reference.class);
    ...
    private static <T> T convert(Class<T> c, Pointer<?> ptr, Bridge bridge) {
        if (ptr == null) {
            // FIXME: null pointer means that this is the static/dummy instance
            throw new IllegalArgumentException("Converting static/dummy instance");
        }
        ...
        if (bridge.to() == Bridge.class) {
            ...
            try {
                ...
                Pointer<T> tmpp = (Pointer<T>) mh.invoke(ptr);
                pt = tmpp;
            } catch (Throwable t) {
    ...
    // Note: This method is looked up and used through a MethodHandle by the type conversion methods generated below
    // FIXME: This could all be baked into the method handle as a series of guardWithTest instead
    private static <T> T typeConverter(TypeConversion[] conversions, Reference<?> ref, Class<T> c) {
        Pointer<?> ptr = ref.ptr();

        for (TypeConversion tc : conversions) {
            if (tc.to() == c) {
                return convert(c, ptr, tc.using());
            }
        }

        throw new IllegalArgumentException("Illegal target class " + c);
    }

    private void generateTypeConversionMethod(ClassGeneratorContext ctxt, Method method) {
        // FIXME: Is null acceptable here?
        TypeConversion[] conversions = null;

        if (method.isAnnotationPresent(TypeConversions.class)) {
            conversions = method.getAnnotation(TypeConversions.class).value();
        } else if (method.isAnnotationPresent(TypeConversion.class)) {
            conversions = new TypeConversion[] { method.getAnnotation(TypeConversion.class) };
        }
        ...
        try {
            mh = MethodHandles
                    .lookup()
                    .findStatic(HeaderImplGenerator.class, "typeConverter", MethodType.methodType(Object.class, TypeConversion[].class, Reference.class, Class.class))
                    .bindTo(conversions);
        } catch (NoSuchMethodException | IllegalAccessException e) {
            throw new RuntimeException(e);
        }
        ...
        MethodGenerator generator = new GenericMethodImplGenerator(ctxt, method.getName(), methodType, false, mh, true);
        generator.generate();
    }

    private void generateSingletonMethod(ClassGeneratorContext ctxt, Method method) {
        MethodHandle mh;

        // FIXME: this should (lazily?) create an instance and return that
        try {
            mh = MethodHandles
                    .publicLookup()
        ...
        MethodType methodType = MethodType.methodType(method.getReturnType(), method.getParameterTypes());
        MethodGenerator generator = new GenericMethodImplGenerator(ctxt, method.getName(), methodType, false, mh, false);
        generator.generate();
    }
    ...
    private void generateMethod(ClassGeneratorContext ctxt, Method method) {
        ...
        } else if (method.isAnnotationPresent(Singleton.class)) {
            generateSingletonMethod(ctxt, method);
        } else if (method.isAnnotationPresent(TypeConversions.class) || method.isAnnotationPresent(TypeConversion.class)) {
            generateTypeConversionMethod(ctxt, method);
        } else if (method.isAnnotationPresent(Bridge.class)) {
            if (!isRecordType) {
                throw new IllegalArgumentException("@Bridge annotation on non record type method " + method);
            }
