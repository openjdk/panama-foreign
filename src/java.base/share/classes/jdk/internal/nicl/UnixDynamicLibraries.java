...
class UnixDynamicLibraries {
    ...
    private UnixDynamicLibraries() {
        try {
            ...
            DLOPEN = MethodHandles.publicLookup().findNative("dlopen", MethodType.methodType(long.class, long.class, int.class));

            // int dlclose(void* handle);
            DLCLOSE = MethodHandles.publicLookup().findNative("dlclose", MethodType.methodType(int.class, long.class));

            // char* dlerror(void);
            DLERROR = MethodHandles.publicLookup().findNative("dlerror", MethodType.methodType(long.class));

            // void* dlsym(void* handle, const char* symbol);
            DLSYM = MethodHandles.publicLookup().findNative("dlsym", MethodType.methodType(long.class, long.class, long.class));
        } catch (Exception e) {
