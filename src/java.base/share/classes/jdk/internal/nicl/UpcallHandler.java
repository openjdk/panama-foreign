...
public class UpcallHandler {
    ...
    private Object boxArgument(Scope scope, UpcallContext context, Reference<?>[] structs, ArgumentBinding binding) throws IllegalAccessException {
        ...
        if (carrierType.isPrimitive()) {
            ...
            Pointer<Long> baseAddress = src.offset(-(binding.getOffset() / LONG_LAYOUT_TYPE.getNativeTypeSize()));

            int nElems = Util.getVectorSize(carrierType) / 8;
    ...
    private void unboxReturn(Class<?> c, UpcallContext context, ArgumentBinding binding, Object o) throws IllegalAccessException {
        ...
        if (c.isPrimitive()) {
            ...
            dst.lvalue().set(addr);
        } else if (Util.isVectorType(c)) {
            long size = Util.getVectorSize(c);
