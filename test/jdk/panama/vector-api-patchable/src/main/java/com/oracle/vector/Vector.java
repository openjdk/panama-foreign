...
//
    ...
    Vector<Integer, S> compareEqual(Vector<E, S> v);

    Vector<Integer, S> lessThan(Vector<E, S> v);

    Vector<Integer, S> greaterThan(Vector<E, S> v);
...
//        Vector<E, S> fromMask(Mask<S> make);
    ...
    interface Mask<S extends Shape<Vector<?, S>>> {
        ...
        default boolean getElement(int i) {
            return toArray()[i];
        }
    }
