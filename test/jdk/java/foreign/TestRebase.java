...
public class TestRebase {
    ...
    public void testRebase(SegmentSlice s1, SegmentSlice s2) {
        if (s1.contains(s2)) {
            ...
            for (int i = 0; i < s2.size(); i++) {
                int expected = (int) BYTE_VH.get(base.offset(i));
                int found = (int) BYTE_VH.get(rebased.offset(i));
            ...
            MemoryAddress rebased = base.rebase(s1.segment);
            for (int i = 0; i < s2.size(); i++) {
                BYTE_VH.get(base.offset(i));
                try {
                    BYTE_VH.get(rebased.offset(i));
    ...
    static Object[][] slices() {
        ...
        for (SegmentSlice.Kind kind : SegmentSlice.Kind.values()) {
            ...
            for (int i = 0 ; i < 16 ; i++) {
                BYTE_VH.set(segment.baseAddress().offset(i), (byte)i);
            }
            //compute all slices
            for (int size : sizes) {
