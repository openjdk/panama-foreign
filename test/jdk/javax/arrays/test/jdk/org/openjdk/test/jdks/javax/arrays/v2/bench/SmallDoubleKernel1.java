...
public class SmallDoubleKernel1 extends Baseline {
    ...
    public static void timingSmall() {
        ...
        set(c, filler);
        long t;
        System.out.println("Matrix mult. times double-textbook vs Double-generic-with-double-accumulator-kernel, milliseconds");
        Double Sum = 0.0;
        double sum = 0.0;
        for (int i = 0; i < 25; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                t = System.currentTimeMillis();
                A.set(DTIMES_kernel1.apply(B,C));
